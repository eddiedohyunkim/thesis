	...
	if (camoGetText.hasChildNodes()) {
  	for (let camoNode of camoGetText.children) {
  		...
  		let camoParagraph = document.createElement('div');
			...
			let fontSize = getComputedStyle(document.querySelector('.camo-paragraph')).getPropertyValue('font-size');
			let lineHeit = getComputedStyle(document.querySelector('.camo-paragraph')).getPropertyValue('line-height');
			...
			for(let letter of camoChildrenText){
				createCamouflage(letter.toLowerCase(), camoParagraph, isItTouch, getBrowser, pushIt);
			}
  	}
	}
